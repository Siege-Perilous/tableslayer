...
- **Write cadence is handled for you.** `queuePropertyUpdate` applies locally at once and flushes to the doc frame-aligned (`$lib/utils/propertyUpdateBroadcaster.ts`: leading edge microtask, trailing edge rAF), and `SessionDocClient` coalesces remote rev bumps to one per frame — so continuous gestures (pans, sliders) can queue per input event. Do not bypass the broadcaster with per-event `client.write.*` calls in a gesture loop; one broadcast per Y transaction means unthrottled writes flood remote peers.
