<script lang="ts">
  import type { PartyPlan, SelectParty } from '$lib/db/app/schema';
  import { useStripeCheckout } from '$lib/queries';
  import { handleMutation } from '$lib/factories';
  import { Button, Link, Panel, Spacer, Text, Title } from '@tableslayer/ui';
  ...
  const handleUpgrade = async (plan: PartyPlan) => {
    await handleMutation({
      mutation: () => $stripeCheckout.mutateAsync({ partyId: party.id, plan }),
      toastMessages: {
...
<Panel class="partyUpgrade">
  {#if party.plan !== 'free'}
    ...
    <div class="partyUpgrade__buttons">
      ...
      <Button onclick={() => handleUpgrade('lifetime')} variant="special">$60 - lifetime</Button>
    </div>
