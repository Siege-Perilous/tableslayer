...
export const createImageJSON = async (file: File) => {
  ...
  const imageUrl = URL.createObjectURL(new Blob([fileBuffer], { type: fileType }));
  image.src = imageUrl;

  // Wait until the image is loaded to get correct dimensions
...
export const readImageJSON = async (json: string) => {
  const { data, fileType, fileSize, width, height } = JSON.parse(json);
