...
CREATE TABLE `__new_marker` (
	...
	`name` text DEFAULT 'New token' NOT NULL,
	`text` text,
	`image_location` text,
	...
	CONSTRAINT "protected_marker_visibility" CHECK("__new_marker"."visibility" >= 0 AND "__new_marker"."visibility" <= 2),
...
INSERT INTO `__new_marker`("id", "scene_id", "visibility", "name", "text", "image_location", "image_scale", "position_x", "position_y", "shape", "shape_color", "size") SELECT "id", "scene_id", "visibility", "name", "text", "image_location", "image_scale", "position_x", "position_y", "shape", "shape_color", "size" FROM `marker`;--> statement-breakpoint
