...
CREATE TABLE `__new_marker` (
	...
	`note` text,
	`pinned_tooltip` integer DEFAULT false NOT NULL,
	...
	CONSTRAINT "protected_marker_shape" CHECK("__new_marker"."shape" >= 0 AND "__new_marker"."shape" <= 4),
	CONSTRAINT "protected_marker_size" CHECK("__new_marker"."size" >= 1 AND "__new_marker"."size" <= 3)
);
--> statement-breakpoint
INSERT INTO `__new_marker`("id", "scene_id", "visibility", "title", "label", "image_location", "image_scale", "position_x", "position_y", "shape", "shape_color", "size", "note", "pinned_tooltip") SELECT "id", "scene_id", "visibility", "title", "label", "image_location", "image_scale", "position_x", "position_y", "shape", "shape_color", "size", "note", "pinned_tooltip" FROM `marker`;--> statement-breakpoint
