...
export const POST = apiFactory(
  async (event) => {
    try {
      ...
      // If the party doesn't have a Stripe Customer ID, create one
      if (!stripeCustomerId) {
        const customer = await stripe.customers.create({
          name: party.name,
          email: user.email,
          metadata: { partyId, createdByUserId: userId, createdByUserEmail: user.email }
        });

        stripeCustomerId = customer.id;

        // Store the customer ID in the database for future lookups
      ...
      const session = await stripe.checkout.sessions.create({
        ...
        customer: stripeCustomerId, // ✅ Attach existing or new customer
      ...
      // Store the session ID so we can look up purchases later
      await updateParty(partyId, { stripeCheckoutSessionId: session.id });
